...
class RelatorioController extends Controller
{
    ...
    public function exportarUsoMaquinasPdf(Request $request)
    {
        $dataInicio = $request->get('data_inicio', Carbon::now()->startOfMonth()->format('Y-m-d'));
        $dataFim = $request->get('data_fim', Carbon::now()->endOfMonth()->format('Y-m-d'));
        $maquinaId = $request->get('maquina_id');
        ...
        $pdf = Pdf::loadView('relatorios.pdf.uso-maquinas', compact(
            'usos', 'totalHoras', 'totalUsos', 'maquinasMaisUsadas',
            'dataInicio', 'dataFim'
        ));

        return $pdf->download('relatorio-uso-maquinas-' . $dataInicio . '-' . $dataFim . '.pdf');
    }

    public function exportarCustosManutencaoPdf(Request $request)
    {
        $dataInicio = $request->get('data_inicio', Carbon::now()->startOfMonth()->format('Y-m-d'));
        $dataFim = $request->get('data_fim', Carbon::now()->endOfMonth()->format('Y-m-d'));
        $maquinaId = $request->get('maquina_id');
        ...
        $pdf = Pdf::loadView('relatorios.pdf.custos-manutencao', compact(
            'manutencoes', 'custoTotal', 'totalManutencoes', 'custoMedio',
            'custosPorTipo', 'maquinasMaisCustosas', 'dataInicio', 'dataFim'
        ));

        return $pdf->download('relatorio-custos-manutencao-' . $dataInicio . '-' . $dataFim . '.pdf');
    }

    public function exportarProdutividadePdf(Request $request)
    {
        $dataInicio = $request->get('data_inicio', Carbon::now()->startOfMonth()->format('Y-m-d'));
        $dataFim = $request->get('data_fim', Carbon::now()->endOfMonth()->format('Y-m-d'));
